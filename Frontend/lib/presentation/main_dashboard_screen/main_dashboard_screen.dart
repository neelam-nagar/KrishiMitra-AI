...
import '../weather_module_screen/widgets/location_selector_bottom_sheet.dart'
    as location_sheet;
...
class _MainDashboardState extends State<MainDashboard>
  ...
  final List<Map<String, dynamic>> _moduleData = [
    {'iconName': 'wb_sunny', 'title': 'Weather', 'subtitle': '', 'route': '/weather-detail-screen'},
    {'iconName': 'trending_up', 'title': 'Crop Prices', 'subtitle': 'Latest Mandi Rates', 'route': '/crop-price-screen'},
    {'iconName': 'account_balance', 'title': 'Govt Schemes', 'subtitle': 'View all schemes', 'route': AppRoutes.schemesList},
    {'iconName': 'eco', 'title': 'Organic Farming', 'subtitle': 'Regional Guide', 'route': '/organic-farming-screen'},
    {'iconName': 'shopping_bag', 'title': 'Marketplace', 'subtitle': 'Buy & Sell Crops', 'route': '/marketplace-screen'},
    {'iconName': 'chat_bubble', 'title': 'AI Assistant', 'subtitle': 'KrishiMitra AI', 'route': '/ai-chatbot-screen'},
    {'iconName': 'payments', 'title': 'Compensation', 'subtitle': 'Crop loss & relief', 'route': AppRoutes.compensationHome},
    {'iconName': 'assignment', 'title': 'Land Record', 'subtitle': 'Bhulekh / Apna Khata', 'route': AppRoutes.landRecord},
    {'iconName': 'biotech', 'title_en': 'Crop Disease', 'title_hi': 'फसल रोग पहचान', 'subtitle_en': 'Detect crop disease', 'subtitle_hi': 'फसल का रोग पहचानें', 'route': '/crop-disease-screen'},
    {'iconName': 'account_balance_wallet', 'title_en': 'Kisan Loan', 'title_hi': 'किसान ऋण', 'subtitle_en': 'Apply for loan', 'subtitle_hi': 'ऋण के लिए आवेदन करें', 'route': '/kisan-loan-screen'},
  ];
  ...
  // FIX: removed unused _checkLocationPermission method

  Future<void> _detectLocation() async {
    try {
      if (kIsWeb) { _handleLocationTap(); return; }

      bool serviceEnabled = await Geolocator.isLocationServiceEnabled();
      if (!serviceEnabled) { _handleLocationTap(); return; }

      LocationPermission permission = await Geolocator.checkPermission();
      if (permission == LocationPermission.denied) {
        permission = await Geolocator.requestPermission();
        if (permission == LocationPermission.denied) { _handleLocationTap(); return; }
      }
      if (permission == LocationPermission.deniedForever) { _handleLocationTap(); return; }
      ...
      if (placemarks.isNotEmpty) {
        ...
        final district = (place.locality ?? place.subAdministrativeArea ?? '').trim();
        if (district.isNotEmpty) {
          if (!mounted) return;
          context.read<LocationProvider>().updateLocation(
            district: district, tehsil: '', village: '',
  ...
  Future<void> _handleRefresh() async {
    ...
    setState(() { _isRefreshing = false; _lastUpdated = DateTime.now(); });
    if (mounted) {
  ...
  void _showLocationDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () { Navigator.pop(context); _handleLocationTap(); },
            child: Text(lang == 'en' ? 'Select Manually' : 'मैन्युअली चुनें'),
          ),
          TextButton(
            onPressed: () async { Navigator.pop(context); await _detectLocation(); },
                ...
                Padding(
                  ...
                  child: WeatherPreviewCard(
                    weatherData: _weatherData == null
                        ...
                        : {..._weatherData!, 'location': _getDisplayLocation(locationProvider.fullLocation)},
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(
                        lang == 'en' ? 'Last updated: ${_formatTime(_lastUpdated)}' : 'अंतिम अपडेट: ${_formatTime(_lastUpdated)}',
                        style: theme.textTheme.bodySmall?.copyWith(
                          color: Colors.white,
                          shadows: const [Shadow(blurRadius: 3, color: Colors.black45, offset: Offset(0, 1))],
                        ),
                      ),
                    ],
                  ),
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    // FIX: removed unused locationProvider local variable
    ...
    return MainShellScreen(
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: [
            Positioned.fill(
              child: Image.asset('assets/images/farm_bg.jpg', fit: BoxFit.cover),
            ),
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(lang == 'en' ? 'Updating dashboard...' : 'डैशबोर्ड अपडेट हो रहा है...'),
                        ],
                      ),
                    )
                  : TabBarView(
                      ...
                      children: [
                        ...
                        EmptyStateWidget(
                          ...
                          message: lang == 'en' ? 'Browse and list agricultural products' : 'कृषि उत्पाद देखें और बेचें',
                          actionText: lang == 'en' ? 'Explore Marketplace' : 'बाजार देखें',
                          onActionTap: () => _handleModuleTap('/marketplace-screen'),
                        ),
                        EmptyStateWidget(
                          iconName: 'chat_bubble',
                          title: lang == 'en' ? 'AI Assistant' : 'कृषि सहायक',
                          message: lang == 'en' ? 'Get instant farming guidance from KrishiMitra AI' : 'कृषि मित्र AI से तुरंत सलाह पाएं',
                          actionText: lang == 'en' ? 'Start Chat' : 'चैट शुरू करें',
                          onActionTap: () => _handleModuleTap('/ai-chatbot-screen'),
                        ),
                      ],
                    ),
            ),
